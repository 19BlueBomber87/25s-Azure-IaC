...
# sudo tshark 
# sudo tshark -i eth0 -f "host 172.40.0.4" --color
...
# Get-NetFirewallRule -Name WINRM-HTTP-In-TCP
# winrm quickconfig;Enable-PSRemoting;Set-Item WSMan:\localhost\Client\TrustedHosts -Value *

# az group create -n wtf -l WestUS2
# az vm create --resource-group wtf  --name wtf --public-ip-sku Standard --image "Win2022Datacenter" --vnet-address-prefix 172.16.0.0/16 --subnet-address-prefix 172.16.0.0/24  --admin-username user01 --admin-password "P@ssw0rd2025!" --generate-ssh-keys --nsg-rule NONE 
# $vmNSG = "wtfNSG"

# az network nsg rule create --resource-group wtf --nsg-name $vmNSG --name yahoo-allow-RDP --protocol tcp --priority 101 --destination-port-range 3389 --access Allow

# #Enable PS Remoting
# Set-AzVMExtension -ResourceGroupName wtf -ExtensionName "PSRemoting" -VMName wtf -Location "WestUS2" -Publisher Microsoft.Compute -ExtensionType CustomScriptExtension -TypeHandlerVersion 1.8 -SettingString '{"commandToExecute":"powershell winrm quickconfig -quiet;Enable-PSRemoting -Force;Set-Item WSMan:\\localhost\\Client\\TrustedHosts -Value * -Force"}' -Verbose *>&1


# az vm create --resource-group azfund04-Server2022 --name win11 --public-ip-sku Standard --image "MicrosoftWindowsDesktop:office-365:win11-23h2-avd-m365:22631.5039.250311" --admin-username admin01 --admin-password "P@ssw0rd2025!" --size 'Standard_B1s' --generate-ssh-keys --nsg-rule RDP  --accept-term 

# az vm create --resource-group azfund04-Server2022 --name svr22 --public-ip-sku Standard --image "Win2022Datacenter" --admin-username admin01 --admin-password "P@ssw0rd2025!" --size 'Standard_B2ms' --generate-ssh-keys --nsg-rule RDP  --accept-term 
# Get-AzVMSize
